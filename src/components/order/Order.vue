      ...
      <el-card>
        ...
        <div class="table-box">
          ...
          >
            ...
            <el-table-column prop="realName" min-width="80" label="购买人">
            </el-table-column>
            <el-table-column prop="phoneNumber" width="120" label="联系电话">
            </el-table-column>
            <el-table-column prop="totalPrice" width="80" label="订单总价">
              <template slot-scope="scope">
                ¥&nbsp;{{ scope.row.totalPrice }}
              </template>
            </el-table-column>
            <el-table-column prop="deposit" width="90" label="已交保证金">
              <template slot-scope="scope">
                ¥&nbsp;{{ scope.row.deposit }}
              </template>
            </el-table-column>
            <el-table-column prop="balancePrice" width="80" label="剩余尾款">
              <template slot-scope="scope">
                ¥&nbsp;{{ scope.row.balancePrice }}
              </template>
            </el-table-column>
            <el-table-column prop="createDate" width="110" label="订单创建时间">
            </el-table-column>
            <el-table-column prop="payDate" width="110" label="订单支付时间">
            </el-table-column>
            <el-table-column prop="messageStatus" width="100" label="支付状态">
              <template slot-scope="scope">
                <el-tag v-if="scope.row.payStatus === '0'" type="warning"
                  >未支付</el-tag
                >
                <el-tag v-if="scope.row.payStatus === '2'" type="danger"
                  >用户取消</el-tag
                >
...
export default {
  ...
  methods: {
    ...
    //         type: 'warning'
    //     }).then(async () => {
    //         let { data : res } = await this.$request.post(`/system/order/deleteDataInfos`, deleteList)
    //         if(res.code == 1) {
    ...
    async getTableData() {
      ...
      // 查询数据
      let { data: res } = await this.$request.post(
        `/system/order/getTableDataList/${this.page}/${this.pageSize}`,
        this.queryParam
      );
      // 查询成功
